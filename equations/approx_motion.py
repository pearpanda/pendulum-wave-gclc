...
if __name__ == '__main__':
    solver = PendulumSolver(degree=6)

    t0 = pi / 3
    lengths = np.linspace(0.25, 0.5, 10)

    polynomials = [solver.solve(l, t0) for l in lengths]
    periods = [solver.period(t0, l) / 2 for l in lengths]

    with open('equations/data/pendulums.txt', 'w') as file:
        for i in range(len(lengths)):
            file.write(f'{lengths[i]}\n')
            file.write(f'{periods[i]}\n')  # {abs(periods[i] / 2 - sqrt(lengths[i]))}
            file.write(f'{polynomials[i].tolist()}\n')
